...
## Collaborator feedback response (single problem, not many)

**Concern:** “From the overview it seems we are combining multiple problems. Primary problem is video-to-text / video summarization.”

**Answer:** We have **one** primary problem:

> **Crash video → faithful text explanation** (domain-specific video summarization / video-to-text).

Everything else (participants, events, TECs, interventions) is **how we generate and evaluate that text**, not a second task family. Anticipation remains dropped.

| Layer | Role | Separate paper problem? |
|-------|------|-------------------------|
| Video → text generation | **Primary task** | No — this *is* the task |
| Video LLM backbone | Method for generation | No — SOTA engine |
...
### Phase A — finalize video-to-text backbone (collaborator priority)

1. Treat the task as **video summarization / explanation generation** on Crash-1500 (`Explanation` + structured fields).
2. Compare Methods 1–3 below; **adopt Method 3 (Video LLM) as default backbone**.
...
7. Evidence interventions (remove / keep-only / remove-irrelevant) + HAG.
8. Multi-Video-LLM benchmark; optional light preference alignment (not BLEU LoRA).

**Recommended title (video-to-text first wording):**  
*Faithful Crash Video Summarization with Temporal Evidence Contracts*

**Alternative:** *From Video to Verifiable Crash Reports: Evidence-Grounded Causal Reasoning for Traffic Accidents*

**Full mathematical formulation:** [README.md](README.md#mathematical-formulation)

---

## SOTA video-to-text methods (answer to Methods 1–3)

### Method 1 — Independent frame captioning + post-merge

- **Idea:** Caption each frame independently → concatenate → summarize.
- **Status:** Classical / weak. Loses motion; duplicates; no true temporal reasoning.
- **Limitation:** Cannot reliably describe “vehicle enters then collides.”
- **Our use:** **Weak baseline only** (not the method).

### Method 2 — Sequential / recurrent context

- **Idea:** Frame $t$ conditioned on summary or hidden state of frames $<t$.
- **Status:** Historically used (RNN/LSTM captioners). Largely **absorbed** into modern Video LLM attention over sampled frames.
- **Limitation:** Error accumulation; weaker than full video transformers.
- **Our use:** Optional ablation (“streaming summary”), not primary.

### Method 3 — Native Video LLMs (current SOTA backbone) — **SELECT THIS**

- **Idea:** Sample frames/clips → vision encoder → temporal position encodings → LLM text decoder.
- **How Video LLMs work internally (2025–2026):**
  - **Sampling:** fixed or dynamic FPS (e.g. Qwen2.5-VL).
  - **Vision path:** SigLIP / ViT / InternViT → visual tokens; often temporal patch / 3D-conv compression (VideoLLaMA3; Qwen tubes).
  - **Temporal encoding:**
    - **MRoPE** (Qwen2/2.5-VL): IDs = *(temporal, height, width)*; Qwen2.5 aligns temporal IDs with **absolute time**.
    - **Textual timestamps** (Qwen3-VL): e.g. `<3.0 seconds>` before patches for grounding / dense captioning.
  - **Decoder:** Qwen2.5 / Qwen3-class LLM generates the summary from interleaved vision+time tokens.
...
- **Gap we fill:** crash forensic **video-to-text** with claim→interval contracts + remove-evidence tests (not only general hallucination QA).

```mermaid
flowchart LR
  VideoIn[CrashVideo] --> VidLLM[Method3_VideoLLM]
  VidLLM --> Draft[DraftSummary_Text]
  Draft --> Claims[ClaimDecomposition]
  Claims --> TEC[EvidenceContracts]
  TEC --> Verify[VerifyOrAbstain]
  Verify --> Final[FaithfulCrashSummary]
  TEC --> Intervene[FrameInterventions]
  Intervene --> Metrics[ERS_IRS_HAG]
```
...
- **Completed pipeline:** LLaVA-NeXT zero-shot + LoRA + ablations + NLI (`PROJECT_COMPLETION_SUMMARY.md`) — use as motivating failure (BLEU≠faithfulness)
- **Seed for epistemic labels:** ~103 non-empty `Ambiguity` notes
- **Drop from main paper:** anticipation / TTA / early-warning — diagnostics only
...
### A0. Video-to-text / Video LLM / hallucination (generation backbone)

#### Qwen2.5-VL / Qwen3-VL (Alibaba, 2025)
- **Method:** Native video tokens + **MRoPE** (absolute-time temporal IDs) / textual timestamps; dynamic FPS; LLM decoder for captioning & grounding
- **Limitation:** Strong open generators; still not crash-evidence-verified summaries
- **How we use:** **Primary Method-3 backbone** for video→text

#### VideoLLaMA3 (2025)
- **Method:** SigLIP + video compressor + Qwen2.5 LLM; streaming / temporal grounding data
- **Limitation:** General video understanding; no claim-level crash contracts
- **How we use:** Strong open Method-3 baseline

#### VidHalluc / EventHallusion / ARGUS / VideoHallucer (2025)
- **Method:** Benchmarks for temporal / object hallucination; show QA≠generation faithfulness
- **Limitation:** Often binary QA or general domains; not forensic crash summaries with frame-removal tests
- **How we solve:** TEC + interventions on **free-form crash summaries**

#### GRAVITI / DINO-HEAL / VER (2025)
- **Method:** Training-free grounding or evidence rewards for Video LLMs
- **Limitation:** General hallucination mitigation; not structured crash TECs
- **How we follow:** Training-free verify path; differ via crash contracts + interventions
